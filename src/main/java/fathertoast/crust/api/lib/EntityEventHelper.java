...
/**
 * Entity events. Sent from the server using {@link World#broadcastEntityEvent(Entity, byte)}, then
...
public enum EntityEventHelper {
    ...
    /** Spawns honey block particles via {@link net.minecraft.block.HoneyBlock#showSlideParticles(Entity)}. */
    HONEY_SLIDE_PARTICLES( 53 ), // Note: This is actually the only event defined in Entity
    /** Spawns honey block particles via {@link net.minecraft.block.HoneyBlock#showJumpParticles(Entity)}. */
    HONEY_JUMP_PARTICLES( 54 ),
    
    /**
     * Spawns item particles and plays the break sound as applicable. Consider using the built-in hook
     * {@link LivingEntity#broadcastBreakEvent(EquipmentSlotType)} or {@link LivingEntity#broadcastBreakEvent(Hand)}.
     */
    ...
    /**
     * Spawns item particles and plays the break sound as applicable. Consider using the built-in hook
     * {@link LivingEntity#broadcastBreakEvent(EquipmentSlotType)}.
     */
