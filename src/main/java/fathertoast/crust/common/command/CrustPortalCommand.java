...
public class CrustPortalCommand {
    ...
    public static final String NAME = ICrustApi.MOD_ID + "portal";
    ...
    private static int run( CommandSourceStack source, PortalBuilder portalBuilder, Entity target ) {
        if( !portalBuilder.isValidDimension( target.level() ) ) {
            CrustCmdHelper.sendFailure( source, NAME, "dimension" );
            return -1;
        }
        ...
        if( failed ) {
            CrustCmdHelper.sendFailure( source, NAME, "place" );
            return 0;
        }
        
        portalBuilder.generate( target.level(), currentPos, forward );
        CrustCmdHelper.sendSuccess( source, NAME, "place", CrustObjects.PORTAL_REGISTRY.get().getKey( portalBuilder ),
